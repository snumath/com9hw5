...
static void update_state(bool_t update_mem, bool_t update_cc)
{
    ...
    */
    ...
    flag = set_word_val(mem, mem_addr, mem_data);
	...
	flag = set_byte_val(mem, mem_addr, mem_data);
	//if (!set_byte_val(mem, mem_addr, mem_data)) {
	if (!flag) {
	    sim_log("\tCouldn't write to address 0x%llx\n", mem_addr);
	} else {
	    sim_log("\tWrote 0x%llx to address 0x%llx\n", mem_data, mem_addr);
		...
		if (mem_addr % 8 != 0) {
		    ...
		    word_t align_addr = mem_addr & ~0x3;
		    word_t val;
		    get_word_val(mem, align_addr, (byte_t *)&val);
		    set_memory(align_addr, val);
		    align_addr+=8;
		    get_word_val(mem, align_addr, (byte_t *)&val);
		    set_memory(align_addr, val);
...
void do_mem_stage()
{
    ...
    if (mem_byte)
      ...
      dmem_error = dmem_error || !get_byte_val(mem, mem_addr, (byte_t *)&sink);
	if (dmem_error)
...
/* Set stalling condgen_mem_writeitions for different stages */
